...
def get_or_create_oauth_user(db: Session, email: str, name: str = None, avatar_url: str = None) -> str:
    ...
    if not user:
        user = User(email=email, password_hash="", name=name, avatar_url=avatar_url)
        db.add(user)
        db.commit()
        db.refresh(user)
    else:
...
async def google_callback(request: Request, db: Session = Depends(get_db)):
    ...
    info = token["userinfo"]
    access_token = get_or_create_oauth_user(
        db,
        email=info["email"],
        name=info.get("name"),
...
async def github_callback(request: Request, db: Session = Depends(get_db)):
    ...
    emails = resp.json()
    # Prefer a real email over GitHub's no-reply privacy address
    email = next(
        (e["email"] for e in emails if e["primary"] and e["verified"] and "noreply" not in e["email"]),
        None
    ) or next(
        (e["email"] for e in emails if e["verified"] and "noreply" not in e["email"]),
        None
    ) or next(
    ...
    print(f"GitHub login: using email {email}")
